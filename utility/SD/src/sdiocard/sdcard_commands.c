...
const uint32_t sdcard_tran_speed_units[7] = {10, 100, 1000, 10000, 0, 0, 0};
const uint32_t sdcard_tran_speed_multipliers[16] = {0, 10, 12, 13, 15, 20, 25, 30, 35, 40, 45, 50, 55, 60, 70, 80};
...
bool sdcard_cmd6_switch_func(sdcard_t * sdcard, sdcard_bus_speed_t bus_speed) {
    ...
    uint8_t buf[SD_SW_STATUS_BSIZE];
    if(sdhc_read_blocks(sdcard->device, buf, SD_SW_STATUS_BSIZE, 1) != SDHC_E_OK)
        return false;
...
bool sdcard_cmd9_send_csd(sdcard_t * sdcard) {
    ...
    uint32_t unit = sdcard_tran_speed_units[csd->sd_v1.TRAN_SPEED & 0x7];
    uint32_t mul = sdcard_tran_speed_multipliers[(csd->sd_v1.TRAN_SPEED >> 3) & 0xF];
