...
void (*SdFile2::dateTime_)(uint16_t* date, uint16_t* time) = NULL;

#if ALLOW_DEPRECATED_FUNCTIONS
  // suppress cpplint warnings with NOLINT comment
  void (*SdFile2::oldDateTime_)(uint16_t& date, uint16_t& time) = NULL;  // NOLINT
#endif  // ALLOW_DEPRECATED_FUNCTIONS
//------------------------------------------------------------------------------
// add a cluster to a file
uint8_t SdFile2::addCluster() {
...
uint8_t SdFile2::addDirCluster(void) {
  if (!addCluster()) {
    return false;
  }
...
dir_t* SdFile2::cacheDirEntry(uint8_t action) {
...
*/
uint8_t SdFile2::contiguousRange(uint32_t* bgnBlock, uint32_t* endBlock) {
  // error if no blocks
  if (firstCluster_ == 0) {
    return false;
  }
...
*/
uint8_t SdFile2::createContiguous(SdFile2* dirFile,
...
/**
   ...
   the value zero, false, is returned for failure.
*/
uint8_t SdFile2::dirEntry(dir_t* dir) {
...
*/
void SdFile2::dirName(const dir_t& dir, char* name) {
  uint8_t j = 0;
...
*/
void SdFile2::ls(uint8_t flags, uint8_t indent) {
  dir_t* p;
...
uint8_t SdFile2::make83Name(const char* str, uint8_t* name) {
  uint8_t c;
...
/** Make a new directory.
   ...
   \return The value one, true, is returned for success and
   the value zero, false, is returned for failure.
   Reasons for failure include this SdFile2 is already open, \a dir is not a
   directory, \a dirName is invalid or already exists in \a dir.
*/
uint8_t SdFile2::makeDir(SdFile2* dir, const char* dirName) {
  dir_t d;
...
/**
   ...
   \return The value one, true, is returned for success and
   the value zero, false, is returned for failure.
   Reasons for failure include this SdFile2 is already open, \a difFile is not
   a directory, \a fileName is invalid, the file does not exist
   or can't be opened in the access mode specified by oflag.
*/
uint8_t SdFile2::open(SdFile2* dirFile, const char* fileName, uint8_t oflag) {
  uint8_t dname[11];
  dir_t* p;
...
*/
uint8_t SdFile2::open(SdFile2* dirFile, uint16_t index, uint8_t oflag) {
  // error if already open
  if (isOpen()) {
    return false;
  }
...
uint8_t SdFile2::openCachedEntry(uint8_t dirIndex, uint8_t oflag) {
...
*/
uint8_t SdFile2::openRoot(SdVolume* vol) {
...
*/
void SdFile2::printDirName(const dir_t& dir, uint8_t width) {
  uint8_t w = 0;
  for (uint8_t i = 0; i < 11; i++) {
    if (dir.name[i] == ' ') {
      continue;
...
*/
void SdFile2::printFatDate(uint16_t fatDate) {
...
*/
void SdFile2::printFatTime(uint16_t fatTime) {
...
*/
void SdFile2::printTwoDigits(uint8_t v) {
  char str[3];
  str[0] = '0' + v / 10;
  str[1] = '0' + v % 10;
  str[2] = 0;
...
/**
   ...
   or an I/O error occurred.
*/
int16_t SdFile2::read(void* buf, uint16_t nbyte) {
...
*/
int8_t SdFile2::readDir(dir_t* dir) {
  int8_t n;
...
dir_t* SdFile2::readDirCache(void) {
...
/**
   ...
   or an I/O error occurred.
*/
uint8_t SdFile2::remove(SdFile2* dirFile, const char* fileName) {
  SdFile2 file;
...
/**
   ...
   the value zero, false, is returned for failure.
*/
uint8_t SdFile2::seekSet(uint32_t pos) {
...
*/
uint8_t SdFile2::sync(uint8_t blocking) {
...
/**
   ...
   the value zero, false, is returned for failure.
*/
uint8_t SdFile2::timestamp(uint8_t flags, uint16_t year, uint8_t month,
...
*/
uint8_t SdFile2::truncate(uint32_t length) {
...
*/
size_t SdFile2::write(const void* buf, uint16_t nbyte) {
...
/**
   ...
   Use SdFile2::writeError to check for errors.
*/
size_t SdFile2::write(uint8_t b) {
  return write(&b, 1);
}
//------------------------------------------------------------------------------
/**
   ...
   Use SdFile2::writeError to check for errors.
*/
size_t SdFile2::write(const char* str) {
...
/**
   ...
   Use SdFile2::writeError to check for errors.
*/
void SdFile2::write_P(PGM_P str) {
...
/**
   ...
   Use SdFile2::writeError to check for errors.
*/
void SdFile2::writeln_P(PGM_P str) {
  write_P(str);
  println();
}
#endif
