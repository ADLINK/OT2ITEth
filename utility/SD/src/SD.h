...
namespace SDLib {
  ...
  class File2 : public Stream {
    private:
      char _name[13]; // our name
      SdFile2 *_file;  // underlying file pointer

    public:
      File2(SdFile2 f, const char *name);     // wraps an underlying SdFile
      File2(void);      // 'empty' constructor
      ...
      File2 openNextFile(uint8_t mode = O_RDONLY);
  ...
  class SDClass {
    ...
    private:
      ...
      SdFile2 getParentDir(const char *filepath, int *indx);
    public:
      ...
      File2 open(const char *filename, uint8_t mode = FILE_READ);
      File2 open(const String &filename, uint8_t mode = FILE_READ) {
    ...
    private:
      ...
      friend class File2;
      friend bool callback_openPath(SdFile2&, const char *, bool, void *);
  };
